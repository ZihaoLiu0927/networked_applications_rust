...
impl KvStoreReader {
    ...
    where F: FnOnce(io::Take<&mut KVDiskReader<File>>) -> Result<R>,
    {
        ...
        if !readers.contains_key(&pos.gen) {
            let r = KVDiskReader::new(
                File::open(logfile!(self.path, pos.gen))?
            )?;
    ...
    fn read_command(&self, pos: &DiskPos) -> Result<Command> {
        self.read_entry_then(pos, 
            |take| {
                Ok(serde_json::from_reader(take)?)
            }
        )
    }
}
...
impl<R: Read+Seek> KVDiskReader<R> {
    ...
    pub fn load_log_from_disk(&mut self, map: &Arc<SkipMap<String, DiskPos>>, fgen: u64) -> Result<u64> {
            ...
            if let Some(entry) = stream.next() {
                ...
                match entry? {
                    Command::Set{key: k, value: _v} => {
                        ...
                        map.insert(k, DiskPos { gen: fgen, pos, len });
                    }
...
impl KvStoreWriter {
    ...
    fn compact(&mut self) -> Result<()> {
        ...
        for entry in self.indexmap.iter() {
            let len = self.reader.read_entry_then(&entry.value(), 
                |mut take| {
                    Ok(io::copy(&mut take, &mut compact_writer)?)
            })?;
            
            self.indexmap.insert(entry.key().clone(), (gen_compact, pos, len).into());
        ...
        self.reader.curr_compact.store(gen_compact, Ordering::SeqCst);
